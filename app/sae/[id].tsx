...
import Animated, {
  ...
  FadeInDown, FadeIn, useAnimatedScrollHandler, interpolate,
...
function GradeBar({ grade, max = 20, t }: {
  grade: number | null; max?: number;
  ...
  useEffect(() => {
    width.value = withSpring(pct, { damping: 14, stiffness: 80 });
  }, []);

  return (
    <View style={[gb.track, { backgroundColor: t.surfaceHigh }]}>
      <Animated.View style={[gb.fill, barAnim]} />
    </View>
  );
}
...
export default function SAEDetailScreen() {
  ...
  const sae = SAE_DATA.find((s) => s.id === id);
  if (!sae) return (
    <View style={[s.root, { backgroundColor: t.bg, paddingTop: insets.top }]}>
      <Text style={[s.notFound, { color: t.textMuted }]}>SAé introuvable.</Text>
    </View>
  );
      ...
      >
        ...
        <Animated.View entering={FadeInDown.delay(100).springify()}>
          <View style={s.statsRow}>
            {[
              { v: String(stats.total),              l: 'étudiants' },
              { v: String(stats.graded),              l: 'notés' },
              { v: String(sae.groups.length),         l: 'groupes' },
              { v: stats.avg?.toFixed(2) ?? '–',      l: 'moyenne', accent: true },
              { v: stats.max?.toFixed(2) ?? '–',      l: 'max' },
              { v: stats.min?.toFixed(2) ?? '–',      l: 'min' },
            ].map((item) => (
        ...
        {sorted.map((g, i) => (
          <Animated.View
            key={g.id}
            entering={FadeInDown.delay(150 + i * 50).springify().damping(14)}
          >
            <View style={[s.groupCard, { backgroundColor: t.surface, borderColor: t.border }]}>
              ...
              <View style={[s.gradeBox, {
                backgroundColor: g.grade !== null
                  ? gradeColor(g.grade, t) + '18'
                  : t.surfaceHigh,
              }]}>
